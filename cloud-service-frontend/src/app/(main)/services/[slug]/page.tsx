...
export default function ServiceDetailsPage() {
    ...
    const detailPlans = useMemo(() => {
        ...
        return matching.length > 0 ? matching : plans.filter(plan => plan.category?.name?.toLowerCase() === config.eyebrow.toLowerCase());
    }, [plans, config]);
            ...
            <section className="relative overflow-hidden bg-background px-gutter py-3xl">
                <div className="relative z-10 mx-auto grid max-w-container-max grid-cols-1 items-center gap-2xl lg:grid-cols-[1.1fr_0.9fr]">
                    <div>
                        ...
                        <p className="mb-sm font-label-caps text-label-caps uppercase tracking-wider text-primary">{config.eyebrow}</p>
                        <h1 className="mb-md font-display-lg text-display-lg text-on-background">{config.title}</h1>
                        <p className="mb-xl max-w-[42rem] font-body-lg text-body-lg text-secondary">{config.description}</p>
                    ...
                    <div className="relative flex min-h-[20rem] items-center justify-center overflow-hidden rounded-2xl border border-outline-variant bg-surface-container-low p-xl shadow-inner">
                        ...
                        <div className="relative z-10 flex h-40 w-40 items-center justify-center rounded-full bg-primary/10"><div className="flex h-28 w-28 items-center justify-center rounded-full bg-primary shadow-[0_0_40px_rgba(0,97,255,0.35)]"><span className="material-symbols-outlined text-6xl text-on-primary">{config.icon}</span></div></div>
                    </div>
                </div>
            </section>

            <section className="mx-auto max-w-container-max px-gutter py-3xl">
                <div className="mb-2xl text-center"><h2 className="mb-sm font-headline-lg text-headline-lg text-on-background">Gói {config.eyebrow}</h2><p className="font-body-md text-body-md text-secondary">Các gói dưới đây được lấy trực tiếp từ dữ liệu dịch vụ đang hoạt động.</p></div>
                <div className="mb-2xl grid grid-cols-1 gap-lg md:grid-cols-2">
                    {config.features.map(feature => <div key={feature} className="flex items-center gap-sm rounded-lg border border-outline-variant bg-surface-container-lowest p-md text-on-surface"><span className="material-symbols-outlined text-primary">check_circle</span>{feature}</div>)}
                </div>
                {isLoading ? <div className="py-2xl text-center text-secondary">Đang tải các gói dịch vụ...</div> : detailPlans.length === 0 ? <div className="rounded-xl border border-outline-variant bg-surface-container-lowest p-xl text-center text-secondary">Hiện chưa có gói {config.eyebrow} đang hoạt động.</div> : <div className="grid grid-cols-1 gap-lg md:grid-cols-2 lg:grid-cols-3">{detailPlans.map(plan => { const price = plan.prices?.find(item => item.isActive !== false); const specs = parseSpecifications(plan.specifications); return <article key={plan.id} className="flex flex-col rounded-xl border border-outline-variant bg-surface-container-lowest p-xl shadow-sm transition hover:-translate-y-1 hover:border-primary hover:shadow-lg"><p className="mb-sm font-label-caps text-label-caps uppercase tracking-wider text-primary">{plan.category?.name || config.eyebrow}</p><h3 className="mb-sm font-headline-md text-headline-md text-on-background">{plan.name}</h3><p className="mb-lg min-h-12 text-secondary">{plan.description || config.description}</p><div className="mb-xl flex-1 space-y-sm border-y border-outline-variant py-md">{specs.length > 0 ? specs.map(([key, value]) => <div key={key} className="flex justify-between gap-md text-sm"><span className="text-secondary">{key}</span><strong>{String(value)}</strong></div>) : <p className="text-sm text-secondary">Liên hệ để nhận thông tin chi tiết.</p>}</div><div className="flex items-center justify-between gap-md"><span className="font-semibold text-primary">{price ? `${price.price.toLocaleString("vi-VN")} đ/${price.billingCycle === "Yearly" ? "năm" : "tháng"}` : "Liên hệ"}</span><Link href={`/pricing?plan=${plan.id}`} className="rounded-lg bg-primary px-md py-sm text-sm font-semibold text-on-primary hover:bg-primary/90">Đăng ký gói</Link></div></article>; })}</div>}
            </section>
