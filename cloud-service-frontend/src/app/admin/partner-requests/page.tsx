...
export default function AdminPartnerRequestsPage() {
    ...
    const handleExport = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`/api/PartnerRequests/export?search=${encodeURIComponent(searchKeyword)}&status=${encodeURIComponent(statusFilter)}`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
            if (res.ok) {
                ...
                a.href = url;
                a.download = `DanhSachYeuCau_${new Date().getTime()}.xlsx`;
            ...
            <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                    <button 
                        ...
                        className="bg-surface text-secondary border border-outline-variant px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-surface-container-low transition-all flex items-center gap-2 disabled:opacity-50"
                    >
