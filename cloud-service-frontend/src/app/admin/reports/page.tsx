...
export default function AdminReportsPage() {
    ...
    const handleExport = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`/api/Admin/revenue-report/export?period=${encodeURIComponent(period)}`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
            if (res.ok) {
                ...
                a.href = url;
                a.download = `BaoCaoDoanhThu_${period}.xlsx`;
        ...
        <div className="max-w-container-max mx-auto space-y-lg pb-xl">
            <div className="flex justify-between items-end">
                ...
                <div className="flex gap-sm">
                    ...
                    <button disabled={isExporting} onClick={handleExport} className="px-md py-sm bg-primary text-on-primary rounded-lg font-body-sm text-body-sm font-medium hover:bg-primary-container transition-colors shadow-sm flex items-center gap-sm disabled:opacity-50">
